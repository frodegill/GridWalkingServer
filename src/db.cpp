...
bool persistGridsAndBonuses(Poco::Data::Session* session_in_transaction, const std::string& guid, const restbed::Bytes& body)
{
	if (body.empty())
		return true;
	...
	uint32_t bonus;
	if (fetch_uint32(iter, end, bonus) && 0xFFFFFFFF!=bonus)
	{
		DEBUG_TRY_CATCH(*session_in_transaction << "UPDATE user SET bonus=? WHERE guid=?",
	...
	uint32_t bonus_pos;
	while (fetch_uint32(iter, end, bonus_pos) && 0xFFFFFFFF!=bonus_pos)
	{
		DEBUG_TRY_CATCH(*session_in_transaction << "INSERT INTO bonus (owner, bonus) VALUE (?,?)",
			Poco::Data::Keywords::use(user_id),
			Poco::Data::Keywords::use(bonus_pos),
			Poco::Data::Keywords::now;)
	}
	return true;
}
...
bool persist2(const std::string& guid, const Poco::UInt32* levels, Poco::UInt32 score, const std::string& name, const restbed::Bytes& body)
{
	if (0 == score)
		return true;
	...
	bool success = persistGridsAndBonuses(session_in_transaction, guid, body);
	DB.ReleaseSession(session_in_transaction, success ? gridwalking::PocoGlue::COMMIT : gridwalking::PocoGlue::ROLLBACK);
	return success;
}

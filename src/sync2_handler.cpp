...
void sync2_handler(const std::shared_ptr<restbed::Session> session)
{
	...
	else
	{
		...
		if (length > 0)
		{
			...
			session->fetch(length, [param_guid,level,score,param_name](const std::shared_ptr<restbed::Session> session, const restbed::Bytes& body)
			{
				...
				int response_status;
				std::string response_body;
				if (persist2(param_guid, level, score, param_name, body) &&
